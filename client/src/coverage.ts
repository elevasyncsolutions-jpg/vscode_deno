...
export async function showTestCoverage(): Promise<void> {
  ...
  const denoCommand = await getDenoCommandPath();
  if (!denoCommand) {
  ...
  const watcher = vscode.workspace.onDidChangeTextEditorVisibleRanges((e) => {
